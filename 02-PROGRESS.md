...
- **Voice, both directions, web app only.** Mic button in the chat input (`webapp/src/components/ChatInput.jsx`) records via `MediaRecorder`, uploads to `POST /api/sessions/{id}/voice-messages`, which transcribes locally with `faster-whisper` (`app/speech.py`, multilingual `small` model, CPU/int8 — no cloud call, same reasoning as never wiring a cloud LLM: voice can carry the same household/health context) and feeds the transcript through the same `chat_service.run_turn()` as typed messages. Replies to voice messages auto-play via the browser's native `SpeechSynthesis` API (`webapp/src/tts.js`); every assistant message also gets a manual 🔊 replay button, hover-reveal on desktop / always-visible on mobile (same touch-accessibility pattern as the rename/archive icons). Verified live end-to-end with real synthesized speech through the actual HTTP endpoint, not just unit-level. `faster-whisper` needed `requests` added explicitly to `requirements.txt` — its own dependency chain doesn't pull it in despite importing it directly. Telegram voice input/output is a separate, larger piece (no client-side speech APIs there) — not started.

### Test harness + CI (new — closes issue #4)
- `tests/` (pytest, 90 tests) against a dedicated `lifeos_test` Postgres database — created automatically, table-truncated between tests, never touches the real dev DB. Covers `chat_service` (mocked Ollama), `finance_client`'s bill-rollover/budget-alert math, `tools.py`'s tool-dispatch table, the full `/api/sessions` CRUD+messaging API, that `alembic upgrade head` applies cleanly against a brand-new database, and now `calendar_client`/`maps_client`/`health_client` too (see below).
